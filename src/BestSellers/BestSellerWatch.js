...
const BestSellerWatch = () => {
    ...
    useEffect(() => {
        axios.get("https://e-mqem.onrender.com/BestSellers/BestSellerWatch")
            ...
            <div id='best-sellers'>
                ...
                <div className='bestseller-products-data-container'>
                    <div className='bestseller-products-data'>
                        {
                            data.slice(0, visible).filter((item, index) => { return item.id >= 346 && item.id <= 360 }).map((element, index) => (
